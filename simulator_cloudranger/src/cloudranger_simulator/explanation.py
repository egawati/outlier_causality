...
from EasyRCA.baselines.cloudranger import cloud_ranger0
...
def run_cloudranger_explain_outlier(window_size, 
						   slide_size, 
						   slide_number,
						   causal_graph, 
						   ex_window_queue,
	...
	while(True):
		try:
			...
			if outliers is None and swindow is None:
				...
				print(f'FINDING {len(outliers)} outliers ')
				...
				print(f'outlier_index {outlier_index}')
				...
				if data.shape[0] > 0 and len(out_event_ts) > 0:
					...
					print(f'outliers_start_time {outliers_start_time}')
					start = time.perf_counter()
					root_causes = list()
					for outlier_start_time in outliers_start_time:
						res = cloud_ranger0(causal_graph, 
											data, 
											anomalous_nodes=list(data.columns), 
...
class ExplainerCloudRangerMP(Process):
	def __init__(self,
			    window_size, 
			    slide_size, 
			    slide_number,
