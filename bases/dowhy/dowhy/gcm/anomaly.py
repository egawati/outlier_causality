...
def attribute_anomalies(
    ...
    if anomaly_scorer is None:
        ...
        print(f'anomaly_scorer is {anomaly_scorer}')

    print(f'target_node {target_node}')
    noise_of_anomaly_samples = compute_noise_from_data(causal_model, anomaly_samples)
    # print(f'anomaly_samples is {anomaly_samples}')
    # print('noise_of_anomaly_samples')
    # print(noise_of_anomaly_samples)
    ...
    # print('nodes_order')
    # print(f'{nodes_order}')
    # # print('noise_dependent_function')
    # print(noise_dependent_function)

    # print(f'fit anomaly_scorer with node_samples[target_node] {node_samples[target_node]}')
    # print(f'node_samples[target_node].to_numpy().shape {node_samples[target_node].to_numpy().shape}')
    anomaly_scorer.fit(node_samples[target_node].to_numpy())


    # print(f'compute attribute_anomaly_scorers use noise_of_anomaly_samples and noise_samples as inputs to the function')
    # print(f'noise_of_anomaly_samples became so called anomaly_samples')
    # print(f'noise_samples become so called distribution_samples')
    ...
    print(f'attributions is {attributions}')
...
def attribute_anomaly_scores(
    ...
    if attribute_mean_deviation:
        ...
        print(f'the input to anomaly_scoring_func is {anomaly_samples}')
        print(f'compute anomaly scores of the outlier noise -- > anomaly_scores = anomaly_scoring_func(anomaly_samples)')
        anomaly_scores = anomaly_scoring_func(anomaly_samples)
    print(f'---------------------------------------------------------')
    print(f'anomaly_scores of the outlier noise is {anomaly_scores}')
    print(f'distribution_samples which is noise samples is used to generate feature_samples')
    print(f'distribution_samples and all_subsets of nodes possible permutation are used as inputs to the permute_features function')
    print(f'---------------------------------------------------------\n\n')
    print(f'distribution_samples = noise_samples = orginal feature_samples before being permutated')
    print(f'here is what happened when set_function is called')
    print(f'original feature_samples before permutation is {distribution_samples}')
    print(f'when set_function is called, attribute attributions are computed using np.log(_relative_frequency(anomaly_scoring_func(feature_samples) >= anomaly_scores[i]))')
    print(f'---------------------------------------------------------')
    def set_function(subset: np.ndarray) -> Union[np.ndarray, float]:
        # print(f'subset which is the input to set_function is {subset}')
        feature_samples = permute_features(distribution_samples, np.arange(0, subset.shape[0])[subset == 0], True)
        # print(f'feature_samples is {feature_samples}')
        # print(f'feature_samples.shape is {feature_samples.shape}')
        #print(f'type of feature permutation  feature_sample is {type(feature_samples)} and its shape is {feature_samples.shape}')
        ...
        for i in range(anomaly_samples.shape[0]):
            ...
            if attribute_mean_deviation:
                ...
                # print(f'sum is {np.sum(anomaly_scoring_func(feature_samples) >= anomaly_scores[i])}')
                # print(f'anomaly_scoring_func(feature_samples) {anomaly_scoring_func(feature_samples).shape}')
                result[i] = np.log(_relative_frequency(anomaly_scoring_func(feature_samples) >= anomaly_scores[i]))
                #print(f'anomaly_scoring_func(feature_samples).shape is {anomaly_scoring_func(feature_samples).shape}')
        return result
