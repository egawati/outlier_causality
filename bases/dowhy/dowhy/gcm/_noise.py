...
def compute_data_from_noise(causal_model: StructuralCausalModel, noise_data: pd.DataFrame) -> pd.DataFrame:
    ...
    for node in sorted_nodes:
        if is_root_node(causal_model.graph, node):
            ...
            # print(f'parent of {node} is {get_ordered_predecessors(causal_model.graph, node)}')
...
def get_noise_dependent_function(
    ...
    """
    validate_causal_dag(causal_model.graph)
    print(f'causal_model.graph {causal_model.graph}')
    ...
    if approx_prediction_model is not None:
        ...
        print(f'get_noise_dependent_function, NO approx_prediction_model')
        print(f'node_connected_subgraph_view(causal_model.graph, target_node)')
        print(node_connected_subgraph_view(causal_model.graph, target_node))
        graph = node_connected_subgraph_view(causal_model.graph, target_node)
        print(type(graph))
...
def noise_samples_of_ancestors(
    ...
    for node in sorted_nodes:
        ...
        if is_root_node(causal_model.graph, node):
            ...
            #print(f'{node} is a root node, therefore noise_samples and node_samples are the same {drawn_noise_samples[node] == drawn_samples[node]}')
        else:
            ...
            # print(f'{node} is a child node, therefore noise_samples and node_samples are not the same {drawn_noise_samples[node] == drawn_samples[node]}')
            # print(f'in this case first noise_samples are generated')
            # print(f'and then node_samples are generated from evaluating _parent_samples_of(node, causal_model, drawn_samples) and noise_samples')
